...
if ($options->get('usecount')) {
    ...
    if (!$options->get('useorderby')) {
        uasort($list, 'compare_by_count');
    }
} else {
...
function doOutput($list, $ph)
{
    ...
    foreach ($list as $one) {
        $uri   = $one['value'];
        $count = " (" . $one['count'] . ")";
        if (startsWith($uri, "http://dbpedia.org/resource/") || startsWith($uri, "http://dbpedia.org/property/")
        ) {
            $prefixeduri = str_replace(
                "http://dbpedia.org/resource/", "", str_replace("http://dbpedia.org/property/", "", $uri)
            );
        } else {
...
function cache($queryoptimizer, $q, $prefixHandler)
{
    ...
    if (isset($_SESSION['patterns']) && $_SESSION['patterns'] == $patterns && startsWithIgnoreCase($q, $_SESSION['q'])
    ) {
