...
class TaggingController extends OntoWiki_Controller_Component
{
    ...
    /**
     * List all tags of a class
     */
    public function exploretagsAction() {
        //require_once 'Erfurt/Sparql/SimpleQuery.php';

        // used for Ajax requests so no layout needed
        $this->_helper->viewRenderer->setNoRender();
        $this->_helper->layout()->disableLayout();
         ...
         */
        $tagging = $this->set_Parameters();

        // shortening
        $class = $tagging['currentClass'];

        if ($this->model == null) {
            require_once 'OntoWiki/Exception.php';
            throw new OntoWiki_Exception('Please select knowledge base first');
            exit;
        } else if (empty($class) || $class == 'none') {
            require_once 'OntoWiki/Exception.php';
            throw new OntoWiki_Exception('Please select class first');
            exit;
        }
        ...
        if (is_array($this->session->cloudproperties)) {
            $cloudproperty = $this->session->cloudproperties[0];
        } else {
            $cloudproperty = $conf->tagproperty;
        }

        $options = array(
            'rdf_type' => (string) $this->_owApp->selectedClass,
        ...
        /*
         * Query, to get all tags for a class
         */
        ...
        $tagVar = new Erfurt_Sparql_Query2_Var('tag');
        $trVar = new Erfurt_Sparql_Query2_Var('tr');
        $query->addTriple(new Erfurt_Sparql_Query2_Var('resourceUri'),
            new Erfurt_Sparql_Query2_IriRef($cloudproperty),
            $trVar);
        /*$query->addTriple(
         $trVar,
         new Erfurt_Sparql_Query2_IriRef($conf->tagname),
         $tagVar
         );*/
        //$query->addProjectionVar($tagVar);
        $query->addProjectionVar($trVar);
        #echo htmlentities($query);

        if (empty($tagging['selectedTags'])) {
            $allTagresources = array();

            $titleHelper = new OntoWiki_Model_TitleHelper($this->model);

            $tempTr = $model->sparqlQuery($query);
            foreach ($tempTr as $singleTR) {
                if (!isset($allTagresources[$singleTR['tr']])) {
                    $titleHelper->addResource($singleTR['tr']);
                    $allTagresources[$singleTR['tr']] = "";
                }
            }

            foreach ($allTagresources as $tr => $name) {
                $allTagresources[$tr] = $titleHelper->getTitle($tr);
            }

            // there isn't any tags
            if (empty($allTagresources)) {
                $this->tagCloudFailed();
            } else {
                sort($tempTr);
                # var_dump($tempTr);die;
                ...
                for ($i = 0; $i < sizeof($tempTr); $i++) {
                    if ($tempTr[$i] != end($tempTr)) {
                        if (isset($tempTr[$i+1]) &&
                        $tempTr[$i] == $tempTr[$i+1]) {
                            $count++;
                        } else {
                            $allWeightedTags[$tempTr[$i]['tr']] = array(
                                'tag' => $allTagresources[$tempTr[$i]['tr']],
                                'weight' => $count);
                            $count = 1;
                        }
                    } else {
                ...
                # var_dump($allWeightedTags);die;

                // get min and max value of tag occurrences
                // $_minValue
                // $_maxValue
                $weightArray = array();
                foreach ($allWeightedTags as $url=>$v) {
                    $weightArray[$url] = $v['weight'];
                }

                $_minValue = min($weightArray);
                $_maxValue = max($weightArray);

                // set font size steps
                $minFontSize = $conf->tagweightsMin;
                $maxFontSize = $conf->tagweightsMax;

                $tagsWithFontSize = array();

                foreach ($allWeightedTags as $uri => $value) {
                    $weight = $this->calcWeight($_minValue, $_maxValue,
                    $minFontSize, $maxFontSize, $value['weight']);
                    $tagsWithFontSize[] = array(
                        'uri' => $uri,
                        'tag' => $value['tag'],
                        'weight' => $weight,
                ...
                // sort by frequency, to get the most
                // important cooccurrence-tags
                usort($tagsWithFontSize, array( $this, 'compare_weight'));

                // get only 5, 10, 20 or custom number of tags
                $getTagByCount = array();
                if ($tagging['count'] <= count($tagsWithFontSize)) {
                    usort($tagsWithFontSize, array( $this, 'compare_weight'));
                    for ($i = 0; $i< $tagging['count']; $i++) {
                        $getTagByCount[] = $tagsWithFontSize[$i];
                    }
                } else {
                    usort($tagsWithFontSize, array( $this, 'compare_weight'));
                    for ($i = 0; $i< count($tagsWithFontSize); $i++) {
                        $getTagByCount[] = $tagsWithFontSize[$i];
                    }
                }
                # var_dump($getTagByCount);die;

                // sorting the tags by name or count
                if ($tagging['sort'] == 'name') {
                    usort($getTagByCount, array( $this, 'compare_label'));
                } else if ($tagging['sort'] == 'count') {
                    usort($getTagByCount, array( $this, 'compare_weight'));
                }

                //$this->view->tags = $getTagByCount;
                $this->view->tags = array($cloudproperty => $getTagByCount);
                //$this->view->selectedTags = $tagging['selectedTags'];
                // var_dump($this->view->selectedTags);

                $etTemplate = 'tagging/exploretags.phtml';
                $this->_response->setBody($this->view->render($etTemplate));
            }
        } else {
            $allTagresources = array();
            $num = 1;
            ...
            /*
             * get all tagresources and tags of a class,
             * which are cooccurrences of selected tags.
             * selectedTags are also included.
             */
            foreach ($tagging['selectedTags'] as $tr) {
                $query->getWhere()->addElement(new Erfurt_Sparql_Query2_Triple(
                new Erfurt_Sparql_Query2_Var('resourceUri'),
                new Erfurt_Sparql_Query2_IriRef($conf->tagproperty),
                new Erfurt_Sparql_Query2_IriRef($tr)
                ));
            }
            # echo htmlentities($query);

            /*
             * for each selected tag construct a statement for the query
             * ex. ' ?r <ns9:taggedWithTag ?tagresource1 . >
             *       ?tagresource1 <ns9:name> "selectedTagA" '
             * this part will be used later, to find all tags
             * which arre used toghether with the selected tags
             */
            ...
            // get distinct tagss
            foreach ($tempTr as $singleTR) {
                $allTagresources[$singleTR['tr']] = $singleTR['tag'];
            }
            # var_dump($allTagresources);die;

            // there isn't any tags (selected or cooccurrences)
            if (empty($allTagresources)) {
                $this->tagCloudFailed();
            } else {
                sort($tempTr);
                ...
                # var_dump($tagsWithFontSize);die;
                foreach ($tagging['selectedTags'] as $skey => $svalue) {
                    foreach ($tagsWithFontSize as $key => $value) {
                        if ($svalue == $value['uri']) {
                            $tagsWithFontSize[$key] = array(
                                'uri' => $value['uri'],
                                'tag' => $value['tag'],
                                'weight' => $value['weight'],
                                'selected' => 'select');
                ...
                // sort by selected tags to split in two arrays:
                usort($tagsWithFontSize, array($this, 'compare_selected'));

                // the number of selected tags
                $numSelTags = count($tagging['selectedTags']);

                // split the array
                // 1. the first one are unselected tags
                // 2. the second contains the selected tags
                $splitValue = count($tagsWithFontSize) - $numSelTags;
                $unselected = array_slice($tagsWithFontSize, 0, $splitValue);
                $selected = array_slice($tagsWithFontSize, $splitValue);

                // get only 5, 10, 20 or custom number of tags
                $getTagByCount = array();
                if ($tagging['count'] <= count($tagsWithFontSize)) {
                    usort($unselected, array( $this, 'compare_weight'));
                    for ($i = 0; $i< $tagging['count']-$numSelTags; $i++) {
                        $selected[] = $unselected[$i];
                    }
                } else {
                    usort($tagsWithFontSize, array( $this, 'compare_weight'));
                    for ($i = 0; $i< count($tagsWithFontSize); $i++) {
                        $selected[] = $tagsWithFontSize[$i];
                    }
                }

                // get unique multidimensional array
                foreach ($this->compareByUri($selected) as $key => $value) {
                    $alltags[] = $value;
                }

                // sorting the tags by name or count
                if ($tagging['sort'] == 'name') {
                    usort($alltags, array( $this, 'compare_label'));
                } else if ($tagging['sort'] == 'count') {
                    usort($alltags, array( $this, 'compare_weight'));
                }

                //$this->view->tags = $alltags;
                $this->view->tags = array($cloudproperty => $alltags);
                $this->view->selectedTags = $tagging['selectedTags'];
                // var_dump($this->view->selectedTags);
