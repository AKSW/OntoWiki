...
class VideolinkPlugin extends OntoWiki_Plugin
{
   ...
   public function init()
    {
        ...
        // load styles
        $this->view->headLink()->appendStylesheet($this->_pluginUrlBase . 'resources/styles/videolink.css','screen, projection');
    ...
    public function onDisplayObjectPropertyValue($event)
    {
       ...
       if (in_array($event->property,  $this->_privateConfig->properties->toArray(), true))
       {
           //create player ID
            ...
            $this->view->divId    = 'videoObjectProperty' . $playerId;

            // render the template
            $html = $this->view->render('video.phtml');
    ...
    public function onDisplayLiteralPropertyValue($event)
    {
        if (in_array($event->property,  $this->_privateConfig->properties->toArray(), true))
        {
            ...
            $this->view->playerSrc = $this->_pluginUrlBase . 'resources/swf/player-viral.swf';
            $this->view->uri       = $event->value;
            $this->view->divId    = 'videoLiteralProperty' . $playerId;
