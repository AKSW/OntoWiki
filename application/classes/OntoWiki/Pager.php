...
{   
     ...
     */
    public static function get($count, $limit = null, $itemsOnPage = null)
    {
        if (null != $limit) {
        ...
        if (($count > $limit) || ($count == Erfurt_Store::COUNT_NOT_SUPPORTED)) {
            ...
            if ($page > 1) {
                if (self::$_options['show_first_last']) {
                    ...
                    $pagerLinks[] = sprintf('<a href="%s">%s</a>', self::$_url, self::$firstHtml . $translate->_('First'));
                }

                self::$_url->{self::$_options['page_param']} = $page - 1;
                $pagerLinks[] = sprintf('<a href="%s">%s</a>', self::$_url, self::$prevHtml . $translate->_('Previous'));
            } else {
                if (self::$_options['show_first_last']) {
            ...
            if ($count != null) {
                if (self::$_options['show_first_last']) {
                    ...
                    if ($page == 1) {
                        ...
                        $pagerLinks[] = '<a href="' . self::$_url . '">1</a>';
                    }
                    $offset = 1;
                ...
                if ($count === Erfurt_Store::COUNT_NOT_SUPPORTED) {
                    for ($i=max(1+$offset, $page-$maxLinksAsym); $i <= $page; ++$i) {
                        ...
                        if ($page == $i) {
                            ...
                            $pagerLinks[] = '<a href="' . self::$_url . '">' . $i . '</a>';
                        }
                    }
                } else {
                ...
                if (!self::$_options['show_first_last'] && ($count !== Erfurt_Store::COUNT_NOT_SUPPORTED)) {
                    ...
                    if ($page == self::$_url->{self::$_options['page_param']}) {
                        ...
                        $pagerLinks[] = '<a href="' . self::$_url . '">' . self::$_url->{self::$_options['page_param']} . '</a>';
                    }
                }
            }
            ...
            if (($count > $page * $limit) || ($count == Erfurt_Store::COUNT_NOT_SUPPORTED && $itemsOnPage === $limit)) {
                self::$_url->{self::$_options['page_param']} = $page + 1;
                $pagerLinks[] = sprintf('<a href="%s">%s</a>', self::$_url, $translate->_('Next') . self::$nextHtml);
                
                if (self::$_options['show_first_last']) {
                    self::$_url->{self::$_options['page_param']} = (int) ceil($count / $limit);
                    $pagerLinks[] = sprintf('<a href="%s">%s</a>', self::$_url, $translate->_('Last') . self::$lastHtml);
                }
            } else {
